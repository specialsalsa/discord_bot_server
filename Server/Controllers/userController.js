...
const kickUser = (req, res) => {
    ...
    sequelize
    .query("UPDATE user SET kicked =  WHERE user_id = :user_id;", {
        replacements: {
            user_id: req.sanitize(req.params.id),
...
const updateUser = (req, res) => {
    ...
    sequelize
    .query("UPDATE user SET username = :username, kicked = :kicked_status WHERE user_id = :user_id;", {
        replacements: {
            user_id: req.sanitize(req.params.id),
            username: req.sanitize(req.body.username),
            delete_status: req.sanitize(req.body.deleted)
...
const addUserToChannel = (objReq, callback) => {
    sequelize.query('INSERT user_channel (id_user, id_channel) VALUES (:ids);',
    {
        replacements: {
